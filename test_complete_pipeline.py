...
def test_complete_pipeline():
    ...
    class MockCDATranslationService:
        ...
        def parse_cda_html(self, html_content):
            ...
            return {"language": "fr", "sections": sections, "patient_info": {}}
        ...
        def create_bilingual_document(self, cda_data):
            ...
            for section, rendered_section in zip(
                ...
                translated_section = {
                    ...
                    "table_data": rendered_section.get(
                        "table_data", {"headers": [], "rows": []}
                    ),
                }
                translated_sections.append(translated_section)

            return {"source_language": "fr", "sections": translated_sections}
    ...
    medication_section = next(
        (s for s in bilingual_data["sections"] if s["section_code"] == "10160-0"), None
    )
    allergy_section = next(
        (s for s in bilingual_data["sections"] if s["section_code"] == "48765-2"), None
    )

    print(f"\n=== Final Results ===")
    if medication_section and medication_section["has_table"]:
        print(
            f"✅ Medications: Found {len(medication_section['table_data']['rows'])} entries"
        )
    else:
    ...
    if allergy_section and allergy_section["has_table"]:
        print(
            f"✅ Allergies: Found {len(allergy_section['table_data']['rows'])} entries"
        )
    else:
