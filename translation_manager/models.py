...
class TerminologySystem(models.Model):
    ...
    version = models.CharField(
        max_length=50, blank=True, help_text="Version of the terminology system"
    )
...
class ConceptMapping(models.Model):
    ...
    source_system = models.ForeignKey(
        TerminologySystem, on_delete=models.CASCADE, related_name="source_mappings"
    )
    ...
    target_system = models.ForeignKey(
        TerminologySystem, on_delete=models.CASCADE, related_name="target_mappings"
    )
...
class LanguageTranslation(models.Model):
    ...
    terminology_system = models.ForeignKey(
        TerminologySystem, on_delete=models.CASCADE, related_name="translations"
    )
    ...
    translated_name = models.CharField(
        max_length=500, help_text="Translated term in the specified language"
    )

    # Translation metadata
    is_preferred = models.BooleanField(
        default=False, help_text="Whether this is the preferred translation"
    )
...
class CountrySpecificMapping(models.Model):
    ...
    member_state = models.ForeignKey(
        MemberState, on_delete=models.CASCADE, related_name="concept_mappings"
    )
...
class TranslationService(models.Model):
    ...
    endpoint_url = models.URLField(help_text="URL of the translation service endpoint")
    api_key = models.CharField(
        max_length=200, blank=True, help_text="API key for authentication (if required)"
    )
...
class TranslationCache(models.Model):
    ...
    translation_service = models.ForeignKey(
        TranslationService, on_delete=models.CASCADE, related_name="cached_translations"
    )
    confidence_score = models.FloatField(
        null=True, blank=True, help_text="Confidence score from translation service"
    )
    ...
    expires_at = models.DateTimeField(
        null=True, blank=True, help_text="Optional expiration time for cache entry"
    )
