...
class LanguageTranslationAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Concept Reference', {
            'fields': ('terminology_system', 'concept_code')
        }),
        ('Translation', {
            'fields': ('language_code', 'translated_name', 'is_preferred')
        }),
        ('Metadata', {
            'fields': ('translation_source', 'is_active')
        }),
        ('Timestamps', {
            'fields': ('created_at', 'updated_at'),
            'classes': ('collapse',)
...
class CountrySpecificMappingAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Mapping Quality', {
            'fields': ('mapping_quality', 'is_active')
        }),
        ('Usage Statistics', {
            'fields': ('usage_frequency', 'last_used'),
            'classes': ('collapse',)
        }),
        ('Timestamps', {
            'fields': ('created_at', 'updated_at'),
    ...
    def get_quality_display(self, obj):
        ...
        return format_html(
            ...
            color, obj.get_mapping_quality_display()
        )
...
class TranslationServiceAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Service Information', {
            'fields': ('name', 'service_type', 'endpoint_url')
        }),
        ('Configuration', {
            'fields': (
                'api_key',
...
class TranslationCacheAdmin(admin.ModelAdmin):
    ...
    def get_source_preview(self, obj):
        preview = obj.source_text[:100] + "..." if len(obj.source_text) > 100 else obj.source_text
        return preview
