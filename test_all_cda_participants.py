...
def test_all_eu_cda_participants():
    ...
    for country in sorted(country_folders):
        ...
        if country_results["files_tested"] > 0:
            ...
            print(
                f"      Files with participants: {country_results['files_with_participants']}"
            )
    ...
    print(
        f"Success rate: {(files_with_participants/total_files*100):.1f}%"
        if total_files > 0
        else "N/A"
    )
    ...
    if all_participant_types:
        ...
        for role, count in sorted(
            all_participant_types.items(), key=lambda x: x[1], reverse=True
        ):
            print(f"   {role}: {count}")

    print(f"\n📞 CONTACT INFORMATION COMPLETENESS:")
    print(
        f"   Contacts with phone: {total_contacts_with_phones}/{total_participants} ({(total_contacts_with_phones/total_participants*100):.1f}%)"
        if total_participants > 0
        else "N/A"
    )
    print(
        f"   Contacts with email: {total_contacts_with_emails}/{total_participants} ({(total_contacts_with_emails/total_participants*100):.1f}%)"
        if total_participants > 0
        else "N/A"
    )
    print(
        f"   Contacts with address: {total_contacts_with_addresses}/{total_participants} ({(total_contacts_with_addresses/total_participants*100):.1f}%)"
        if total_participants > 0
        else "N/A"
    )
                ...
                try:
                    ...
                    if namespace:
                        participants = root.findall(
                            './/hl7:participant[@typeCode="IND"]', ns
                        )
                    else:
                    ...
                    for participant in participants:
                        ...
                        if namespace:
                            function_code_elem = participant.find(
                                ".//hl7:functionCode", ns
                            )
                        else:
                        ...
                        if namespace:
                            entity_elem = participant.find(
                                ".//hl7:associatedEntity", ns
                            )
                        else:
                            entity_elem = participant.find(".//associatedEntity")

                        class_code = (
                            entity_elem.get("classCode")
                        ...
                        # Track codes
                        if function_code:
                            coded_analysis["function_codes"][function_code] = (
                                coded_analysis["function_codes"].get(function_code, 0)
                                + 1
                            )
                        if class_code:
                            coded_analysis["class_codes"][class_code] = (
                                coded_analysis["class_codes"].get(class_code, 0) + 1
                            )
                        if relationship_code:
                            coded_analysis["relationship_codes"][relationship_code] = (
                                coded_analysis["relationship_codes"].get(
                                    relationship_code, 0
                                )
                                + 1
                            )
    ...
    for i, combo in enumerate(list(coded_analysis["unique_combinations"])[:5]):
        ...
        print(
            f"   Pattern {i+1}: typeCode='{type_code}', functionCode='{func_code}', classCode='{class_code}', relationshipCode='{rel_code}'"
        )
