            ...
            if hasattr(response, "context_data"):
                ...
                if "patient_identity" in context:
                    ...
                    if actual_patient_id == expected_patient_id:
                        ...
                        print(
                            f"❌ Patient ID mismatch: expected {expected_patient_id}, got {actual_patient_id}"
                        )
                        return False
                else:
...
def test_session_loss_handling():
    ...
    try:
        ...
        try:
            response = patient_details_view(request, test_patient.id)
            print(
                "✅ Patient details view executed successfully with missing session data"
            )
            ...
            if hasattr(response, "context_data"):
                ...
                if "session_error" in context and context["session_error"]:
                    print(
                        f"✅ Session error properly detected: {context['session_error']}"
                    )
                    return True
                else:
