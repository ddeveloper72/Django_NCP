"""
Django management command to test FHIR Conditions Extractor.

Usage:
    python manage.py test_fhir_conditions

Tests extraction of Condition resources from Portuguese FHIR bundle.
"""
...
class Command(BaseCommand):
    help = 'Test FHIR Conditions Extractor with Portuguese bundle'

    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('\n' + '='*80))
        self.stdout.write(self.style.SUCCESS('FHIR CONDITIONS EXTRACTOR TEST'))
        ...
        extractor = FHIRConditionsExtractor()
        
        # Extract conditions section
        self.stdout.write('Extracting conditions section...\n')
        ...
        if section_data["has_entries"]:
            self.stdout.write(self.style.SUCCESS('='*80))
            self.stdout.write(self.style.SUCCESS('CONDITION ENTRIES'))
            self.stdout.write(self.style.SUCCESS('='*80 + '\n'))
            ...
            for i, entry in enumerate(entries, 1):
                ...
                self.stdout.write(f'   Clinical Status: {entry["clinical_status"]}')
                self.stdout.write(f'   Verification Status: {entry.get("verification_status", "N/A")}')
                
                if entry.get("table_data"):
                    td = entry["table_data"]
                    if td.get("severity"):
                        self.stdout.write(f'   Severity: {td["severity"]}')
                    if td.get("onset_date"):
                        self.stdout.write(f'   Onset Date: {td["onset_date"]}')
                    if td.get("abatement_date"):
                        self.stdout.write(f'   Resolved Date: {td["abatement_date"]}')
                    if td.get("category"):
                        self.stdout.write(f'   Category: {td["category"]}')
        ...
        self.stdout.write(f'\nVerification Colors:')
        for status, color in display_config["verification_colors"].items():
            self.stdout.write(f'   - {status}: {color}')
        ...
        total_checks += 1
        if section_data["section_id"] == "problems":
        ...
        # Check 2: Expected number of entries (Portuguese bundle has 7 conditions)
        total_checks += 1
        expected_count = 7
        ...
        # Check 4: All entries have clinical status
        total_checks += 1
        ...
        # Check 5: All entries have verification status
        total_checks += 1
        all_have_verification = all(entry.get("verification_status") for entry in section_data["clinical_table"]["entries"])
        if all_have_verification:
            self.stdout.write(self.style.SUCCESS('Success: All entries have verification status'))
            checks_passed += 1
        else:
            self.stdout.write(self.style.ERROR('Error: Some entries missing verification status'))
        ...
        output_file = Path(settings.BASE_DIR) / 'test_fhir_conditions_output.json'
