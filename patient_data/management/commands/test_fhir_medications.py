"""
Django management command to test FHIR Medications Extractor.

Usage:
    python manage.py test_fhir_medications

Tests extraction of MedicationStatement resources from Portuguese FHIR bundle.
"""
...
class Command(BaseCommand):
    help = 'Test FHIR Medications Extractor with Portuguese bundle'

    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('\n' + '='*80))
        self.stdout.write(self.style.SUCCESS('FHIR MEDICATIONS EXTRACTOR TEST'))
        ...
        extractor = FHIRMedicationsExtractor()
        
        # Extract medications section
        self.stdout.write('Extracting medications section...\n')
        ...
        if section_data["has_entries"]:
            self.stdout.write(self.style.SUCCESS('='*80))
            self.stdout.write(self.style.SUCCESS('MEDICATION ENTRIES'))
            self.stdout.write(self.style.SUCCESS('='*80 + '\n'))
            ...
            for i, entry in enumerate(entries, 1):
                ...
                if entry.get("table_data"):
                    td = entry["table_data"]
                    if td.get("indication"):
                        self.stdout.write(f'   Indication: {td["indication"]}')
                    if td.get("start_date"):
                        self.stdout.write(f'   Start Date: {td["start_date"]}')
                    if td.get("dosage_text"):
                        self.stdout.write(f'   Dosage: {td["dosage_text"]}')
                    if td.get("route"):
                        self.stdout.write(f'   Route: {td["route"]}')
        ...
        total_checks += 1
        if section_data["section_id"] == "medications":
            self.stdout.write(self.style.SUCCESS('✅ Section ID is correct'))
            checks_passed += 1
        else:
            self.stdout.write(self.style.ERROR(f'❌ Section ID incorrect: {section_data["section_id"]}'))

        # Check 2: Expected number of entries (Portuguese bundle has 5 medications)
        total_checks += 1
        expected_count = 5
        if section_data["entry_count"] == expected_count:
            self.stdout.write(self.style.SUCCESS(f'✅ Entry count matches expected ({expected_count})'))
            checks_passed += 1
        else:
            self.stdout.write(self.style.WARNING(f'⚠️ Entry count: {section_data["entry_count"]}, expected: {expected_count}'))
        ...
        if all_have_display:
            self.stdout.write(self.style.SUCCESS('✅ All entries have display text'))
            checks_passed += 1
        else:
            self.stdout.write(self.style.ERROR('❌ Some entries missing display text'))
        ...
        if all_have_status:
            self.stdout.write(self.style.SUCCESS('✅ All entries have clinical status'))
            checks_passed += 1
        else:
            self.stdout.write(self.style.ERROR('❌ Some entries missing clinical status'))

        # Check 5: Has clinical codes
        total_checks += 1
        if len(clinical_codes) > 0:
            self.stdout.write(self.style.SUCCESS(f'✅ Clinical codes extracted ({len(clinical_codes)} codes)'))
            checks_passed += 1
        else:
            self.stdout.write(self.style.WARNING('⚠️ No clinical codes extracted'))

        # Check 6: Table columns configured
        total_checks += 1
        if len(columns) >= 4:
            self.stdout.write(self.style.SUCCESS(f'✅ Table columns configured ({len(columns)} columns)'))
            checks_passed += 1
        else:
            self.stdout.write(self.style.ERROR(f'❌ Insufficient table columns: {len(columns)}'))

        # Check 7: Data structure matches CDA format
        total_checks += 1
        ...
        if has_all_keys:
            self.stdout.write(self.style.SUCCESS('✅ Data structure matches CDA format'))
            checks_passed += 1
        else:
            self.stdout.write(self.style.ERROR('❌ Data structure incomplete'))

        # Summary
        self.stdout.write(f'\n' + '='*80)
        if checks_passed == total_checks:
            self.stdout.write(self.style.SUCCESS(f'✅ ALL VALIDATION CHECKS PASSED! ({checks_passed}/{total_checks})'))
        else:
            self.stdout.write(self.style.WARNING(f'⚠️ VALIDATION: {checks_passed}/{total_checks} checks passed'))
        self.stdout.write('='*80 + '\n')

        # Export to JSON for inspection
        output_file = Path(settings.BASE_DIR) / 'test_fhir_medications_output.json'
