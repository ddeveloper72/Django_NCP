...
class EnhancedCDATranslationService:
    ...
    def __init__(self, target_language: str = "en"):
        ...
        self.translation_service = (
            TranslationServiceFactory.get_cda_translation_service()
        )
    ...
    def translate_cda_document(
        self, xml_content: str, source_language: str = "fr"
    ) -> TranslatedCDADocument:
        """
        ...
        try:
            ...
            translation_summary = self._generate_translation_summary(
                sections, terminology_translations
            )
    ...
    def _extract_patient_info(self, root: ET.Element) -> Dict:
        ...
        try:
            ...
            if name_elem is not None:
                ...
                patient_info["name"] = (
                    f"{given.text if given is not None else ''} {family.text if family is not None else ''}".strip()
                )
            ...
            # Gender
            gender_elem = root.find(
                ".//cda:patient/cda:administrativeGenderCode", self.namespaces
            )
            if gender_elem is not None:
                patient_info["gender"] = gender_elem.get(
    ...
    def _extract_and_translate_sections(
        self, root: ET.Element, source_language: str
    ) -> List[CDASection]:
        ...
        try:
            ...
            for i, section_elem in enumerate(section_elements):
                section = self._parse_section(
                    section_elem, f"section_{i}", source_language
                )
                if section:
    ...
    def _parse_section(
        self, section_elem: ET.Element, section_id: str, source_language: str
    ) -> Optional[CDASection]:
        ...
        try:
            ...
            original_title = (
                title_elem.text if title_elem is not None else "Untitled Section"
            )
            ...
            # Translate narrative
            translated_narrative = self._translate_narrative(
                original_narrative, source_language
            )
    ...
    def _translate_narrative(self, narrative: str, source_language: str) -> str:
        ...
        translated_text = self._translate_text(
            terminology_result["content"], source_language
        )
    ...
    def _determine_translation_status(
        ...
        orig_narrative: str,
        trans_narrative: str,
        orig_codes: List[Dict],
        trans_codes: List[Dict],
    ) -> str:
        ...
        if has_title_translation or has_narrative_translation or has_code_translations:
            if all(
                [
                    ...
                    has_narrative_translation,
                    has_code_translations,
                ]
            ):
                return "translated"
            else:
    ...
    def _generate_translation_summary(
        self, sections: List[CDASection], terminology_translations: Dict
    ) -> Dict:
        """Generate summary of translation activities"""
        total_sections = len(sections)
        translated_sections = len(
            [s for s in sections if s.translation_status == "translated"]
        )
        partial_sections = len(
            [s for s in sections if s.translation_status == "partial"]
        )
        ...
        return {
            ...
            "translation_coverage": (
                (translated_sections + partial_sections) / total_sections
                if total_sections > 0
                else 0
            ),
    ...
    def _generate_translated_xml(
        self, root: ET.Element, sections: List[CDASection]
    ) -> str:
