...
class MemberState(models.Model):
    ...
    ncp_endpoint = models.URLField(
        help_text="National Contact Point endpoint URL"
    )
...
class PatientIdentifier(models.Model):
    ...
    patient_id = models.CharField(
        max_length=100,
        help_text="Patient ID in the home member state"
    )
    ...
    id_root = models.CharField(
        max_length=100,
        help_text="Root OID for the patient ID"
    )
    created_at = models.DateTimeField(auto_now_add=True)
...
class PatientData(models.Model):
    ...
    break_glass_reason = models.TextField(
        blank=True,
        help_text="Reason for emergency access"
    )
    ...
    raw_patient_summary = models.TextField(
        blank=True,
        help_text="Raw XML patient summary from source NCP"
    )
...
class PatientServiceRequest(models.Model):
    ...
    response_data = models.TextField(
        blank=True,
        help_text="Raw response data from the service"
    )
