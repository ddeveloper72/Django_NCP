    ...
    def get(self, request, patient_id):
        ...
        try:
            ...
            if not cda_document:
                ...
                return render(request, 'patient_data/cda_not_found.html', {
            ...
            context = {
                ...
                'view_mode': request.GET.get('view', 'side-by-side'),  # 'side-by-side', 'toggle', 'translated-only'
            }
...
def get_section_translation(request, patient_id, section_id):
    ...
    try:
        patient_data = get_object_or_404(PatientData, id=patient_id)
        cda_document = request.session.get('cda_documents', {}).get(str(patient_id), {}).get('xml_content')
        
        if not cda_document:
            return JsonResponse({'status': 'error', 'message': 'CDA document not found'}, status=404)
...
def export_translated_cda(request, patient_id):
    ...
    try:
        ...
        if format_type == 'xml':
            ...
            response['Content-Disposition'] = f'attachment; filename="cda_translated_{patient_id}_{target_language}.xml"'
            return response
            ...
            html_content = render(request, 'patient_data/cda_export.html', context).content
            
            response = HttpResponse(html_content, content_type='text/html')
            response['Content-Disposition'] = f'attachment; filename="cda_translated_{patient_id}_{target_language}.html"'
            return response
