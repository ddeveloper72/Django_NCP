...
# from .services.local_patient_search import LocalPatientSearchService  # Disabled
import json
import logging
...
def patient_data_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Log the submission
            logger.info(f"Patient data submitted: {patient_data.given_name} {patient_data.family_name}")
            ...
            credentials = PatientCredentials(
                ...
                birth_date=patient_data.birth_date.strftime('%Y%m%d') if patient_data.birth_date else '',
                gender=patient_data.gender,
                country_code=patient_data.country_code,
                patient_id=patient_data.patient_id or ''
            )
            ...
            return redirect('patient_details', patient_id=patient_data.id)
        else:
            # Form has errors
...
def patient_details_view(request, patient_id):
    ...
    try:
        ...
        return redirect('patient_data_form')
...
def patient_cda_view(request, patient_id):
    ...
    try:
        ...
        return render(request, 'patient_data/patient_cda.html', context)
...
def download_cda_pdf(request, patient_id):
    ...
    try:
        ...
        # TODO: Implement PDF generation from CDA XML
        # For now, return the XML content as plain text
        
        response = HttpResponse(
            match_data['cda_content'], 
            content_type='application/xml'
        )
        response['Content-Disposition'] = f'attachment; filename="patient_cda_{patient_id}.xml"'
...
def local_cda_document_view(request, patient_id, document_index=0):
    ...
    try:
        ...
        try:
            # from .services.cda_parser_service import CDAParserService  # Disabled
        ...
        if convert_to_fhir:
            try:
                # from .services.cda_to_fhir_service import CDAToFHIRService  # Disabled
